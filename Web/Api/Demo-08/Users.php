...
/*
    การทำระบบ User แบบนี้ควรทำใน HTTPS เท่านั้น
    เพราะถ้าใช้ HTTP อาจจะถูกอ่าน Username, Password ระหว่างการรับส่งข้อมูลได้
*/
...
if($_SERVER['REQUEST_METHOD'] === 'POST')
{
    if($has_id === false)
    {
        require_once('_Users/01-Create.php');
        die();
    }
}
else if($_SERVER['REQUEST_METHOD'] === 'GET')
{
    if($has_id === false)
    {
        require_once('_Users/02-Read.php');
        die();
    }
    else
    {
        require_once('_Users/03-ReadWithId.php');
        die();
    }
}
else if($_SERVER['REQUEST_METHOD'] === 'PUT')
{
    if($has_id === true)
    {
        require_once('_Users/04-UpdateWithId.php');
        die();
    }
}
else if($_SERVER['REQUEST_METHOD'] === 'DELETE')
{
    if($has_id === true)
    {
        require_once('_Users/05-DeleteWithId.php');
        die();
    }
}
