...
$order_by_availables = [ 'id', 'username', 'email', 'created_date' ];
$order_by = parse_string($_GET, 'order_by', 'created_date', $order_by_availables);

// สร้าง Array ของชื่อฟิลด์ที่อนุญาตให้ใช้ได้
// string 'id' | 'username' | 'email' | 'created_date'
$field_default = "`id`, `username`, `email`, `created_date`";
$fields = parse_fields($_GET, 'fields', $field_default);

// ตรวจสอบว่า fields มี password หรือไม่ ถ้ามีจะไม่อนุญาต
if(strpos($fields, 'password') !== false)
{
    // 406 : Not Acceptable
